...
const { urlParams: configUrlParams } = config;
...
const dataFetchers = {
  ...
  sessionCount: () => {
    let sessionCount = 1;
    if (dataStore.hasFirstVisitCookie && dataStore.firstVisitData.sessionCount) {
      sessionCount = dataStore.firstVisitData.sessionCount + 1;
    }
    return sessionCount;
  },

  pageCount: () => {
    let pageCount = dataStore.firstVisitData.pageCount || 0;
    pageCount += 1;
    return pageCount;
  },
  ...
  firstVisitDate: () => {
    ...
    const now = new Date();
    const firstVisitDate = `${String(now.getFullYear()).slice(-2)}${String(
      now.getMonth() + 1
